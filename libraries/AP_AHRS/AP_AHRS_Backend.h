...
public:
    ...
    virtual bool EAS(float &airspeed_ret) const WARN_IF_UNUSED { return false; }
    virtual bool EAS(uint8_t airspeed_index, float &airspeed_ret) const { return false; }

    // return a true airspeed estimate (navigation airspeed) if
    // available. return true if we have an estimate
    bool TAS(float &airspeed_ret) const WARN_IF_UNUSED {
        if (!EAS(airspeed_ret)) {
            return false;
        }
