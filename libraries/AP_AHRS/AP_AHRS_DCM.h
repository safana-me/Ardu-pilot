...
public:
    ...
    bool EAS(float &airspeed_ret) const override;

    // return an airspeed estimate if available. return true
    // if we have an estimate from a specific sensor index
    bool EAS(uint8_t airspeed_index, float &airspeed_ret) const override;

    // return a synthetic EAS estimate (one derived from sensors
    // other than an actual airspeed sensor), if available. return
    // true if we have a synthetic airspeed.  ret will not be modified
    // on failure.
    bool synthetic_EAS(float &ret) const WARN_IF_UNUSED {
        ret = _last_TAS;
        return true;
    }
...
private:
    ...
    bool get_unconstrained_EAS_estimate(uint8_t airspeed_index, float &airspeed_ret) const;
