...
void AP_AHRS::update_state(void)
{
    ...
    state.airspeed_ok = _EAS(state.airspeed, state.airspeed_estimate_type);
    state.airspeed_true_ok = _TAS(state.airspeed_true);
    state.airspeed_vec_ok = _TAS(state.airspeed_vec);
...
bool AP_AHRS::_EAS(float &airspeed_ret, AirspeedEstimateType &airspeed_estimate_type) const
{
    ...
    case EKFType::DCM:
        ...
        return dcm.EAS(idx, airspeed_ret);
#endif
...
bool AP_AHRS::_TAS(float &airspeed_ret) const
{
    switch (active_EKF_type()) {
#if AP_AHRS_DCM_ENABLED
    case EKFType::DCM:
        return dcm.TAS(airspeed_ret);
#endif
#if HAL_NAVEKF2_AVAILABLE
    case EKFType::TWO:
#endif
...
bool AP_AHRS::_TAS(Vector3f &vec) const
{
    switch (active_EKF_type()) {
#if AP_AHRS_DCM_ENABLED
    case EKFType::DCM:
