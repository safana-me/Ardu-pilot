...
AP_AHRS_DCM::drift_correction(float deltat)
{
    ...
    if (!have_gps() ||
        ...
        if (airspeed_sensor_enabled()) {
            TAS = AP::airspeed()->get_airspeed() * get_EAS2TAS();
        }
#endif

        // use airspeed to estimate our ground velocity in
        // earth frame by subtracting the wind
        velocity = _dcm_matrix.colx() * TAS;
...
bool AP_AHRS_DCM::EAS(uint8_t airspeed_index, float &airspeed_ret) const
{
    // airspeed_ret: will always be filled-in by get_unconstrained_EAS_estimate which fills in airspeed_ret in this order:
    ...
    if (!get_unconstrained_EAS_estimate(airspeed_index, airspeed_ret)) {
        return false;
    }
...
//               Or if none of the above, fills-in using the previous airspeed estimate
// Return false: if we are using the previous airspeed estimate
bool AP_AHRS_DCM::get_unconstrained_EAS_estimate(uint8_t airspeed_index, float &airspeed_ret) const
{
#if AP_AIRSPEED_ENABLED
    ...
    if (AP::ahrs().get_wind_estimation_enabled() && have_gps()) {
        ...
        airspeed_ret = _last_TAS * get_TAS2EAS();
        return true;
    }
...
Vector2f AP_AHRS_DCM::groundspeed_vector(void)
{
    ...
    Vector2f gndVelGPS;
    float airspeed = 0;
    const bool gotAirspeed = TAS(airspeed);
